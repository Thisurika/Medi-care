      ...
      for (let d = new Date(start); d <= end; d.setDate(d.getDate() + 1)) {
        if (medicine.timing.morning) {
          logsToCreate.push({
            ...
            medicineId: medicine._id || new mongoose.Types.ObjectId(), // Will be generated by mongoose if not passed, but let's just let mongo generate
        ...
        if (medicine.timing.afternoon) {
          logsToCreate.push({
            patient: patientId,
            prescription: createdPrescription._id,
            medicineId: medicine._id || new mongoose.Types.ObjectId(),
            medicineName: medicine.name,
            scheduledDate: new Date(d),
            timeOfDay: 'afternoon',
          });
