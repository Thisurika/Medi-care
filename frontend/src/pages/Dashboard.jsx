...
import { Calendar, FileText, Users, ClipboardList, Star, ChevronRight, Download } from 'lucide-react';
  ...
  return (
    <>
      <div className="page-header-row" style={{ marginBottom: 28 }}>
        ...
        <button className="btn btn-primary" onClick={() => navigate('/services/new')}>Add Service</button>
      </div>
      ...
      <div className="card">
        <div className="section-header">
          ...
          <span className="view-all-link" onClick={() => navigate('/appointments')}>View all</span>
        </div>
      ...
      <div className="two-col-grid">
        ...
        <div className="card">
          <div className="section-header">
            ...
            <span className="view-all-link" onClick={() => navigate('/reports')}>All reports</span>
          </div>
          {reports.length === 0 ? (
            ...
            <div key={r._id} className="card card-hover" style={{ padding: '12px 16px', marginBottom: 10 }}>
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                <div>
                  ...
                  <div style={{ fontSize:12, color:'var(--text-muted)' }}>{r.description?.slice(0,40)}</div>
                </div>
                {r.file_path && (
                  <a href={`http://localhost:5000${r.file_path}`} target="_blank" rel="noopener noreferrer" className="action-link download" style={{ display:'flex', alignItems:'center', gap:4 }}>
